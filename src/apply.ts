...
export async function applySource(
  ...
  // Backup settings.json before making any changes; path returned for rollback
  const backupPath = await backupSettingsJson(ctx)
  ...
  try {
    ...
    if (backupPath) {
      await rollbackSettingsJson(ctx, backupPath)
      throw new RollbackPerformedError(msg, backupPath)
    }
    throw err
  }
}
